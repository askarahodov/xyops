...
class SSO {
	...
	handleSSO(args, callback) {
		...
		this.loadSession( args, function(err, session, user) {
			...
			var external_user = {};
			var header_map = sso.header_map || HEADER_PRESETS[sso.preset || 'NOPE'];
			if (!header_map) {
				// no header map or preset provided
				return self.doSSOError('sso', "SSO Auth Flow Failure: No header map or preset provided.", callback);
			}
			...
			if (sso.cleanup_full_name) {
				...
				external_user.full_name = external_user.full_name.replace(/\@.+$/, '').replace(/\./g, ' ');
			}
