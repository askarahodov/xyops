		...
		this.loadSession(args, function (err, session, user) {
			...
			var finish = function(metadata) {
				if (params.id) {
					...
					if (params.readme) {
						...
						self.request.get( url, function(err, resp, data, perf) {
						...
						self.request.json( url, false, function(err, resp, data, perf) {
			...
			fs.stat( cache_file, function(err, stats) {
				if (err || (stats.mtimeMs / 1000 < Tools.timeNow() - marketplace.ttl)) {
					...
					self.request.json( marketplace.metadata_url, false, function(err, resp, metadata, perf) {
