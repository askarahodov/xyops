...
class Jobs {
	...
	api_download_job_log(args, callback) {
		...
		if (job) {
			...
			var headers = {
				'Content-Type': "text/plain; charset=utf-8",
				'Content-disposition': "attachment; filename=OpsRocket-Partial-Job-Log-" + params.id + '.txt'
			};
			...
			self.storage.getStream( storage_key, function(err, stream) {
				...
				var headers = {
					'Content-Type': "text/plain; charset=utf-8",
					'Content-Encoding': "gzip",
					'Content-disposition': "attachment; filename=OpsRocket-Job-Log-" + params.id + '.txt'
				};
