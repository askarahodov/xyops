...
class Admin {
	...
	api_admin_upgrade_workers(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var job = self.startInternalJob({ title: "Worker server upgrade", type: 'maint', username: user.username || user.id, params });
			var count = 0;
			...
			async.eachSeries( server_ids,
				function(server_id, callback) {
					// sanity check
					if (self.server.shut) {
						self.logError('admin', "Server is shutting down, aborting install job");
					...
					if (!server) {
						self.logError('upgrade', "Server went offline, skipping upgrade: " + server_id);
						return process.nextTick(callback);
					}
					...
					// update job progress
					count++;
					job.update({ progress: count / server_ids.length });
					
					// stagger
					if (params.stagger && (count < server_ids.length)) {
